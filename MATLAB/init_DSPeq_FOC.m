...
% inverter parameters
E = 520; %4                % available voltage
DEADTIME = 100;            % deadtime in multiples of Tpwm

% load parameters
R = 0.47; %0.0307           % phase resistance
L = 3.4e-3; %120e-6         % phase inductance
...
emf = 0.687*we/p;           % machine electromotive force
ph = pi;                    % phase of the load (so that Ed=0)

% IREG parameters
alpha = 0.2; %0.04;         % gain
K1 = alpha*L/Ts;            % constant for IREG
K2 = exp(-Ts/taus);          % parameter that desxcribes system dynamics
Id_ref = 4;                 % reference current in d axis 
Iq_ref = 0;                 % reference current in q axis
...
dmacnt_ref = round(8*taus/Ts);      % regulation period in which reference step change occurs
tref = dmacnt_ref*Ts;               % reference step (from zero to Idq ref)
dmacnt_end = dmacnt_ref*1.5;        % number of regulation periods for simulation to run
